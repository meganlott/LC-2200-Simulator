...
package xyz.room409.lc2200 {
  ...
  //Left panel inputs
    ...
    def updateMem(location: Int, value: Int) = DatapathState.updateMem(location, value)

    //Retreives the value currently on the screen at memory location
    def getMemVal(location: Int) = DatapathState.getMemVal(location)

    //Takes in an integer location and an integer value then updates register location in
    //datapathstate and the UI
    def updateReg(location: Int, value: Int) = DatapathState.updateReg(location, value)

    //Retreives the value currently on the screen at the register location
    def getRegVal(location: Int) = DatapathState.getRegVal(location)
